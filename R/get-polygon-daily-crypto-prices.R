get_polygon_daily_crypto_prices <- function(symbol,from = Sys.Date()-135, to = Sys.Date()) {
  options(scipen = 999)
  ...
  poly_inner <- function(date_pair) {
    ...
    polygon_list <- glue::glue("https://api.polygon.io/v2/aggs/ticker/X:{symbol}/range/1/day/{date_values$value[1]}/{date_values$value[2]}?sort=desc&apiKey={Sys.getenv('polygon')}") %>%
  ...
  if(nrow(return_set) > 0) {
    return_set <- return_set %>% dplyr::mutate(symbol = symbol) %>%
